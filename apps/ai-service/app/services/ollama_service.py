...
class OllamaService:
    def __init__(self) -> None:
        ...
        self.default_model = getattr(settings, "ollama_model", "qwen2.5-coder")
    ...
    def _generate_fallback_text(self, request: ChatRequest, model: str, is_timeout: bool = False) -> str:
        ...
        if ("binary" in prompt_lower or "bit" in prompt_lower) and ("c" in prompt_lower or "c++" in prompt_lower or "code" in prompt_lower):
            guidance = (
            ...
            guidance = "Hello! 👋 I am your **DevSync AI Pair Programmer**. How can I help you write, debug, or refactor code today?"
        else:
            guidance = (
    ...
    async def stream_chat(self, request: ChatRequest) -> AsyncGenerator[str, None]:
        ...
        # Try base_url first, fallback to 127.0.0.1 if localhost resolution hangs
        target_urls = [
        ...
        # Set fast connect timeout (3.0s) so unreachable server doesn't hang for minutes
                ...
                async with httpx.AsyncClient(timeout=http_timeout) as client:
                    async with client.stream("POST", url, json=payload) as response:
                        if response.status_code == 404:
                            # Model not found in Ollama — check for alternative installed models via /api/tags
                            try:
                            ...
                            # If no model is installed yet, stream a clear markdown response with pull instructions
                            help_text = (
                                ...
                                f"ollama pull {model}\n"
                                f"```\n"
                                f"*(Or pull any code model such as `ollama pull codellama` or `ollama pull qwen2.5-coder`).*\n\n"
                            ...
                            err_msg = json.dumps({"content": f"AI Service error ({response.status_code}): {error_detail.decode('utf-8', errors='ignore')}", "done": True})
        ...
        # If all target URLs were unreachable, stream fallback response
